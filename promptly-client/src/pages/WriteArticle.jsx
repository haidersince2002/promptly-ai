import { Copy, Edit, Sparkles, Wand2 } from "lucide-react";
...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const {data} = await axios.post('/api/ai/generate-article', {topic: input, length: selectedLength.length, improvePromptFlag}, {
      ...
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700">
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {articleLength.map((item, index)=>(
            <span onClick={()=> setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 border-gray-300 dark:border-slate-600'}`} key={index}>{item.text}</span>
          ))}
        </div>
        ...
        <div className="mt-5 flex items-center gap-2">
          <label className="relative cursor-pointer">
            ...
            <div className="w-9 h-5 bg-slate-300 dark:bg-slate-600 rounded-full peer-checked:bg-[#4A7AFF] transition"></div>
            <span className="absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition peer-checked:translate-x-4"></span>
          </label>
          <div className="flex items-center gap-1 text-sm">
            <Wand2 className="w-4 h-4 text-[#4A7AFF]"/>
            Improve my prompt
          </div>
        </div>
      ...
      <div className="w-full max-w-lg p-4 bg-white dark:bg-slate-800 rounded-lg flex flex-col border border-gray-200 dark:border-slate-700 min-h-96 max-h-[65vh]">
        ...
        {loading ? (
          <div className="flex-1 flex flex-col justify-center items-center gap-3">
            <span className="w-8 h-8 rounded-full border-3 border-blue-500 border-t-transparent animate-spin"></span>
            <p className="text-sm text-gray-400">Generating your article...</p>
          </div>
        ) : !content ? (
